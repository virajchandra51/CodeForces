...
int helper(int i, int sp, int p, vi &a, int n, vector<vector<vector<int>>> &dp)
{
    if(i==n)return 0;
    if(dp[i][sp+1][p+1]!=-1) return dp[i][sp+1][p+1];
    int ans=0;
    ans=helper(i+1,sp,p,a,n,dp)%M;
    if(sp==-1){
        ans= (ans + helper(i+1,a[i]%2,p,a,n,dp))%M;
    }
    else if(p==-1){
        ans= (ans + helper(i+1,sp,a[i]%2,a,n,dp))%M;
    }
    else{
        if((sp+p+a[i]%2)%2==0)
        ans = (ans+1+helper(i+1,p,a[i]%2,a,n,dp))%M;
    }
    return dp[i][sp+1][p+1]=ans;
}
