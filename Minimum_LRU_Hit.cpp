...
LRUCache::LRUCache(int n) { csize = n; hit = 0;}
...
void solve()
{
    ...
    rep(i,n) cin>>requests[i];
    int ind = 0;
    map<string,int> mp;
    for(auto it:requests)
    {
        if(mp.find(it)==mp.end()) mp[it]=ind++;
    }
    vector<int> v(n); for(int i=0; i<n; i++) v[i] = mp[requests[i]];
    int l = 1;
    int r = n;
    int ans = -1;
    while(l<=r)
