...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        rep(i,q)
        {
            ll l,r,k;
            cin>>l>>r>>k;
            ll countof1 = pre[r]-pre[l-1];
            ll countof2 = pre2[r]-pre2[l-1];
            ll other = r-l+1 - countof1 - countof2;
            // cout<<countof1<<" "<<countof2<<" "<<other<<endl;
            if(countof1<countof2) swap(countof1,countof2);
            if(countof1>=countof2)
            {
                ll diff = countof1-countof2;
                if(k>diff && other>diff)
                {
                    countof2 = countof1;
                    k-=diff;
                    other-=diff;
                    if(k>0 && other>0)
                    {
                        countof1 += (min(other,k)/2);
                        countof2 += (min(other,k)/2+(min(other,k)%2));
                    }
                }
                else if(k>diff)
                {
