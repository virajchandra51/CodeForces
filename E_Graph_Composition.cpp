...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        DSU u1(n),u2(n);
        for(auto it:e2) u2.unionbyRank(it.f,it.s);
        ll ans = 0;
        for(auto it:e1)
        {
            if(u2.findUpar(it.first)==u2.findUpar(it.second))
            u1.unionbyRank(it.f,it.s);
            else ans++;
        }
        set<ll> s1,s2;
        rep(i,n)
        {
            s1.insert(u1.findUpar(i+1));
            s2.insert(u2.findUpar(i+1));
        }
        ans+=(s1.size()-s2.size());
        cout<<ans<<endl;
