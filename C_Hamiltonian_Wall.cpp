...
void solve()
{
    ...
    string a,b;
    cin>>a>>b;
    vector<vector<int>> s(3,vector<int>(n+5,0));
    vector<vector<bool>> dp(3,vector<bool>(n+5,false));
    rep(i,n)
    {
        if(a[i]=='B') s[1][i+1] = 1;
        else s[1][i+1] = 0;
    ...
    if(s[1][n]==1) dp[1][n] = true;
    if(s[2][n]==1) dp[2][n] = true;
    ...
    if(dp[1][1]==1 || dp[2][1]==1) py; else pn;
}
