...
int helper(int i, int j, int n, int m,vector<vector<int>> &a, vector<vector<int>> &dp, vector<vector<int>> &vis)
{
    ...
    rep(k,4)
    {
        ...
        {
            dp[i][j] = max(dp[i][j],1+helper(nx,ny,n,m,a,dp,vis));
        }
    }
    return dp[i][j];
}
...
void solve()
{
    ...
    for(int i=0;i<n;i++){
        for(int j=0;j<m;j++)
        res=max(res,helper(i,j,n,m,a,dp,vis));
    }
    for(auto it:dp)
    {
        cout<<it<<endl;
