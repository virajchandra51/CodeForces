...
void solve()
{
    ...
    vvl dp(n+1,vl(m+1));
    dp[1][1] = a[0][0];
    for(int i=2;i<=n;i++) dp[i][1] = gcd(dp[i-1][1],a[i-1][0]);
    for(int j=2;j<=m;j++) dp[1][j] = gcd(dp[1][j-1],a[0][j-1]);
    vector<vector<pair<ll,ll>>> dp2(n+1,vector<pair<ll,ll>>(m+1));
    for(int i=2;i<=n;i++)
    {
        for(int j=2;j<=m;j++)
        {
            if(i==2 && j==2)
            dp2[i][j] = {gcd(dp[i][j-1],a[i-1][j-1]),gcd(dp[i-1][j],a[i-1][j-1])};
            else if(i==2)
