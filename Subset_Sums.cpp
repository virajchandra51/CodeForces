...
void calculateSum(vector<ll> &v, vector<ll> &a)
{
    int n = a.size();
    for(int i=0;i<(1<<n);i++)
    {
...
void solve()
{
    ...
    calculateSum(v1,arr1);
    calculateSum(v2,arr2);
    sort(all(v2));
    ll c = 0;
    rep(i,v1.size())
    {
        auto it1 = lower_bound(v2.begin(),v2.end(),a-v1[i])-v2.begin();
        auto it2 = upper_bound(v2.begin(),v2.end(),b-v1[i])-v2.begin();
        c+=(it2-it1);
    }
    out(c);
}
