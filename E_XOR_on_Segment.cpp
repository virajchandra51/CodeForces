...
void solve()
{
    ...
    cin>>a;
    cin>>q;
    vector<vector<ll>> bitsArray(n, vector<ll>(20,0));
    vector<LazySGT> segmentTrees;
    rep(i,n) // O(n)
    {
        ll copyN = a[i];
        ll bit = 0;
        while(copyN) //O(log2(copyN))
        {
            if(copyN&1) bitsArray[i][20-bit-1] = 1;
            copyN>>=1;
            bit++;
        }
    }
    for(int i=0; i<20; i++) // O(20)
    {
        ...
        bits.push_back(bitsArray[j][i]);
        segmentTrees.pb(LazySGT(n,bits));
    }
    rep(i,q) // O(q)
    {
        ll operationType;
        ...
        if(operationType == 1)
        {
            ...
            cin>>l>>r;
            ll sum = 0;
            for(int j=0;j<20;j++) // O(20)
            {
                sum+=(pow(2,20-j-1)*(segmentTrees[j].make_query(l-1,r-1).val)); // O(logn)
            }
            cout<<sum<<endl;
        }
        else
        {
            ...
            while(x) // O(log2(x))
            {
                if(x&1) segmentTrees[20-bit-1].make_update(l-1,r-1,1); //O(logn)
                x>>=1;
                bit++;
            }
        }
    } //O(q*20*logn + q*log2(x)*logn) ~ O(q*log2n) 
}
