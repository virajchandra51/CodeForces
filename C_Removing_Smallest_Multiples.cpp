...
void solve()
{
    ...
    rep(i,n)
    {
        ...
        {
            int f=1;
            while(v[(i+1)*f-1].second==0 && ((i+1)*f<=n))
            {
                result+=v[i].first;
                f++;
            }
    ...
    rep(i,zeros.size())
    {
        ...
        while(true)
        {
            bool it = binary_search(all(zeros),factor*zeros[i]);
            if(it==false)
            break;
            if(visited[factor*zeros[i]-1]==0) result+=zeros[i];
            visited[factor*zeros[i]-1]=1;
            factor++;
        }
        if(count(all(visited),1)==visited.size()) break;
    }
    cout<<result<<"\n";
}
