...
void solve()
{
    ...
    rep(i,q)
    {
        ...
        ll cc = 0;
        if (discriminant > 0) {
            long double x1 = (long double)(-b + (sqrt(discriminant*1.0))) / (2.0*a);
            long double x2 = (long double)(-b - (sqrt(discriminant*1.0))) / (2.0*a);
            if(m.find(x1)!=m.end() && m.find(x2)!=m.end() && x1*x2==c)
            cc+=(m[x1]*m[x2]);
        }
