...
struct Node {
	...
	void merge(Node &l, Node &r) { // Merge two child nodes
        ...
        int total = 0;
        for(auto it: combFreq) total+=it.second;
        if(total&1) 
        {
            isGood = 0;
            freq = combFreq;
        ...
        {
            int flag = 1;
            for(auto it: combFreq)
            {
                if(it.second>(total/2)) flag=0;
            }
            isGood = flag;
            freq = combFreq;
        }
...
void solve()
{
    ...
    // {
    //     cout<<i<<endl;
    //     cout<<tree.tree[i].freq<<" isGood: "<<tree.tree[i].isGood<<endl;
    // }
    ll q;
    cin>>q;
    ll tt;
    ...
    rep(i,q)
    {
        ...
        if(type == 1)
        {
            ll l,r;
            cin>>l>>r;
            if(tree.make_query(l-1,r-1).isGood==1) ans+=(r-l+1);
        }
        else
        {
            ll l,r;
            cin>>l>>r;
            tree.make_update(l-1,r);
            // for(int i=0;i<tree.s;i++)
            // {
            //     cout<<i<<endl;
            //     cout<<tree.tree[i].freq<<endl;
            // }
        }
    }
    out(ans)
