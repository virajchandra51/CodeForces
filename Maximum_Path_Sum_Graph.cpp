...
int helper(int node, int par, int &ans, vector<vector<pair<int,int>>> &adj)
{
    int maxi = 0, secmaxi = 0;
    for(auto it:adj[node])
    {
        if(it.first==par) continue;
        int route = it.second+helper(it.first,node,ans,adj);
        ans = max(ans,route);
        if(route>maxi)
        {
            secmaxi = maxi;
        ...
        else if(route>secmaxi)
        {
            secmaxi = route;
        }
    }
    ans = max(ans,maxi+secmaxi);
    return maxi;
}
