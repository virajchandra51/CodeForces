...
void solve()
{
    ...
    map<pair<ll,ll>,pair<ll,ll>> answer;

    rep(i,n) 
    {
        cin>>v[i].f>>v[i].s;
        points.insert({v[i].f,v[i].s});
    }
    ...
    rep(i,n) // n
    {
        rep(j,4) // 4
        {
            ll nx = v[i].f+dx[j];
            ll ny = v[i].s+dy[j];
            if(points.find({nx,ny})==points.end()) // logn
            {
                q.push({v[i].f,v[i].s});
                answer[{v[i].f,v[i].s}] = {nx,ny}; // logn
                break;
            }
        }
    } // pushing with MD 1 
    ...
    while (!q.empty()) {
        ...
        rep(i,4) {
            ll nx = x + dx[i];
            ll ny = y + dy[i];
            if (!points.count({nx, ny}) || answer.count({nx, ny})) {
                continue;
            }
