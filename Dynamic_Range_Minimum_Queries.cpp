...
class SegmentTree{
    public:
        ...
        ll query(ll qL,ll qR, Node* node){
            ...
            else if(node->startInterval>qR || node->endInterval<qL ) return INT_MAX; // node intervals totally outside query
            else
            return min(query(qL,qR,node->left),query(qL,qR,node->right));
        }
        ll update(ll index, ll value, Node* node){
            if(index>=node->startInterval && index<=node->endInterval)
            {
                ...
                {
                    node->data = min(update(index, value, node->left),update(index, value, node->right));
                    return node->data;
                }
            }
            return node->data;
...
void solve()
{
    ...
    rep(i,q)
    {
        ...
        cin>>a>>b>>c;
        if(a==2)
        cout<<tree->query(b-1,c-1,tree->root)<<endl;
        else
        tree->update(b-1,c,tree->root);
    }
}
