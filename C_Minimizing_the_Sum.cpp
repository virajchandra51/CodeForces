...
int32_t main()
{
    ...
    auto solve = [&] () {
        ...
        for(int c=0;c<=k;c++) dp[0][c] = 0;
        for(int pos=1;pos<=n;pos++)
        {
            for(int c=0;c<=k;c++)
            {
                ll mi = 1e18;
                for(int prepos = pos; pos-prepos<=c && prepos>0; prepos--)
                {
                    mi = min(mi,a[prepos-1]);
                    ll x = mi*(pos-prepos+1);
                    x+=dp[prepos-1][c-(pos-prepos)];
                    dp[pos][c] = min(dp[pos][c],x);
                }
            }
        }
        ll ans = 1e18;
        for(int c=0;c<=k;c++) ans=min(ans,dp[n][c]);
        out(ans)
    };
