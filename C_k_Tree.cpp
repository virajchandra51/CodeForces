...
ll helper(ll sum, ll n, ll k, ll d, ll flag, vvl &dp)
{
    if(sum>n) return 0;
    if(flag==1 && sum==n) return 1;
    if(dp[sum][flag]!=-1) return dp[sum][flag]%M;
    ll ans = 0;
    for(ll i = 1; i<=k; i++)
    {
        if(sum+i<=n)
        {
            if(i>=d)
            ans += helper(sum+i,n,k,d,1,dp)%M;
            else ans += helper(sum+i,n,k,d,flag,dp)%M;
        }    
    }
    return dp[sum][flag]=ans%M;
}
