...
void solve()
{
    ll n,m,k;
    cin>>n>>m>>k;
    struct UnionFind u = UnionFind(n+1);
    vector<pair<string,pair<ll,ll>>> queries;
    rep(i,m)
    {
        ll a,b;
        cin>>a>>b;
    ...
    for(int i=queries.size()-1;i>=0;i--) // k
    {
        if(queries[i].first=="ask")
        {
            if(u.get(queries[i].second.first)!=u.get(queries[i].second.second))
            answers.pb("NO");
            else answers.pb("YES");
        }
        else
        {
            u.merge(queries[i].second.first,queries[i].second.second);
        }
    }
    reverse(all(answers)); // answers.size()
    for(auto it:answers) cout<<it<<endl;
}

// tc - (m+k+k*1+answer.size()) - 10^5
// sc - (n+k) 
