...
struct Node {
    ll segSum, prefSum, suffSum, maxSum; // may change
    Node() { // Identity element
        segSum = prefSum = suffSum = maxSum = 0;    // may change
    }
    Node(ll p1) {  // Actual Node
        segSum = p1; // may change
    }
    void merge(Node &l, Node &r) { // Merge two child nodes
        segSum = l.segSum+r.segSum;  // may change
        prefSum = max(l.prefSum, l.segSum + r.prefSum);
        suffSum = max(r.suffSum, r.segSum + l.suffSum);
...
void solve()
{
    ...
    rep(i,q)
    {
        ...
        cout<<tree.make_query(0,n-1).maxSum<<endl;
    }
}
