...
struct Update {
    ...
    void apply(Node &a, int start, int end) { // apply update to given node
        if(val==1)
            a.val = ((end-start+1) - a.val); // may change
    }
...
struct LazySGT {
    ...
    int kthOneQuery(int k, int index, int start, int end)
    {
        if( start == end ) 
            return start;
        pushdown(index, start, end);
        ...
        if(k<leftNode.val)
            return kthOneQuery(k, leftNodeIndex, start, mid);
        else return kthOneQuery(k-leftNode.val, rightNodeIndex, mid+1, end);
    }
};
...
void solve()
{
    ...
    rep(i,q)
    {
        // for(auto it: tree.tree) cout<<it.val<<" ";
        // cout<<endl;
        ll operationType;
        cin>>operationType;
        if(operationType==1)
        {
            ll l,r;
            cin>>l>>r;
            tree.make_update(l,r-1,1);
        }
        else
        {
            ll k;
            cin>>k;
            cout<<tree.kthOneQuery(k,1,0,n-1)<<endl;
        }
