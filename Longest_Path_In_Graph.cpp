...
set<int> bfs2(int u, vector<vector<int>> &adj, int V)
{
    int dis[V];
    memset(dis, -1, sizeof(dis));
    queue<int> q;
...
bool bfs1(vector<vector<int>> &adj, set<int> &sum, set<int> t2, int u, vector<int> &vis)
{
    vis[u]=1;
    if(t2.find(u)!=t2.end())
    {
...
void solve()
{
    ...
    set<int> sum;
    vector<int> vis(n,0);
    bfs1(adj,sum,t2,t1.first,vis);
    ll sss = 0;
    for(auto it:sum) sss+=(it+1);
    out(ss-sss);
}
