...
ll helper(vl &a, ll n, ll x, ll MSB)
{
    ll lastI = -1;
    ll Xor = 0;
    ll Or = 0;
    ...
    {
        Xor ^= a[i];
        ll mask = INT_MAX - ( (1 << (MSB+1)) - 1);
        if((mask&Xor)==(mask&Xor&x) && (Xor&(1<<MSB))==0)
        {
            Or |= Xor;
            Xor = 0;
            sz++;
...
void solve()
{
    ...
    for(int bit = 0; bit<=3; bit++)
        ans = max(ans,helper(a,n,x,bit));
    if(ans==0) pm; else out(ans)
}
